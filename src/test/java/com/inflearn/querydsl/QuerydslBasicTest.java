...
import com.inflearn.querydsl.dto.MemberDto;
import com.inflearn.querydsl.dto.QUserDto;
import com.inflearn.querydsl.dto.UserDto;
...
//                .fetchCount();
    ...
    public void findDtoByJPQL(){
        List<MemberDto> result = em.createQuery("select new com.inflearn.querydsl.dto.MemberDto(m.username, m.age) from Member m", MemberDto.class)
                .getResultList();
    ...
    public void findUserDto(){
        ...
        List<UserDto> result = queryFactory
                .select(Projections.fields(UserDto.class,
                        member.username.as("name"), // 별칭을 준다.

                        ExpressionUtils.as(JPAExpressions // 서브쿼리 사용
                                .select(memberSub.age.max())  // 이름이 없기 때문에 alias 를 준다.
                                .from(memberSub), "age")
                ))
                .from(member)
                .fetch();
    ...
    public void dynamicQuery_BooleanBuilder(){
        ...
        List<Member> result = searchMember1(usernameParam, ageParam); // 들어가는 값에 따라 동적으로 쿼리를 생성한다.
    ...
    public void dynamicQuery_WhereParam(){
        ...
        List<Member> result = searchMember2(usernameParam, ageParam); // 들어가는 값에 따라 동적으로 쿼리를 생성한다.
        assertThat(result.size()).isEqualTo(1);
    }
    ...
    public void bulkUpdate(){
        long count = queryFactory
                .update(member)
                .set(member.username, "비회원") // 비회원으로 이름 변경
                .where(member.age.lt(28))
                .execute();
       // 영속성 컨텍스트에 올라가있음. DB에는 적용되어 있지만, 영속성 컨텍스트에는 적용되어 있지 않음.
       // DB 에서 가져와도 영속성 컨텍스트에 있다면 사용하지 않음.
        // 따라서 영속성 컨텍스트를 초기화 해줘야함.
        em.flush();
        em.clear();
